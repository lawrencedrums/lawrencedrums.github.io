...
async function prettierShuffle() {
  ...
  while (shuffledNumber.length !== arraySize) {
    ...
    if (
      randomIndex !== randomIndexTwo &&
      // And indices are not shuffled...
      shuffledNumber.includes(randomIndex) == false &&
      shuffledNumber.includes(randomIndexTwo) == false
    ) {
...
// Find the position and height of each rectangle in bar view
function getPosBars(i, num) {
  let rectangle = {
...
// Return the position of each rectangle in square view
function getPosSquares(i) {
  let rectangle = {
    x: 200 + ((i % 10) * canvas.width) / 20,
    y: canvas.height - canvas.height + 20 + Math.floor(i / 10) * 72,
    width: (canvas.width - 20) / 20,
    height: (canvas.width - 20) / 20,
  };
